...
When the user clicks the extension toolbar button, the extension inspects the active tab for HTML5 video elements. To display the selected video in the companion macOS helper, the extension may send the following data to the helper app running on the same computer:
